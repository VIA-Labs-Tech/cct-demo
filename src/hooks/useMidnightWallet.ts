/**
 * Midnight wallet connection (connector API v4): discovery, connect, address.
 * Frontend business — everything comes straight from the wallet api.
 */
...
export function useMidnightWallet() {
    ...
    const connect = useCallback(async (wallet: { name: string; label?: string; api: { connect(networkId: string): Promise<MidnightWalletApi> } }) => {
        ...
        try {
            ...
            // Best-effort permission pre-grant; wallets without hintUsage prompt per call.
            try {
                ...
                /* continue — wallet will prompt per call instead */
            }
