/**
 * Frontend-owned CIP-30 helpers: connected address and balances straight from
 * the wallet api — no chain queries, no bridge-package internals.
 */
...
/** CIP-30 returns raw address bytes as hex; display/recipient form is bech32. */
...
/**
 * Minimal CBOR reader for the CIP-30 getBalance() Value: either a plain coin
 * (uint) or [coin, { policyId(bytes) => { assetName(bytes) => quantity } }].
 */
class CborReader {
    private i = 0
    ...
    read(): bigint | Uint8Array | unknown[] | Map<string, unknown> {
        ...
        switch (major) {
            ...
            case 6: // tag — skip and read the tagged value
                this.uint(info)
                return this.read()
            default:
...
export interface CardanoBalances {
    lovelace: bigint
    usdm: bigint
}

/** Decode the wallet's CIP-30 getBalance() CBOR into ADA + USDM amounts. */
export function decodeBalance(balanceCborHex: string): CardanoBalances {
    ...
    if (assets) usdm = assets.get(assetName) ?? 0n
    return { lovelace: coin, usdm }
}
